...
public class PrizeController {
    ...
    // 1. GET ALL hoặc GET BY EVENT ID (Ví dụ: /api/prize?eventId=1)
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping()
    public ResponseEntity<?> getPrizes(@RequestHeader("Authorization") String auth,
                                       @RequestParam(required = false) Long eventId) {
        if (getUid(auth) == null) return unauthorized();

        if (eventId != null) {
            return ResponseEntity.ok(prizeService.getPrizesByEventId(eventId));
        }
        return ResponseEntity.ok(prizeService.getAllPrizes());
    }

    // 2. GET BY ID - Xem chi tiết 1 giải thưởng
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/{id}")
    ...
    public ResponseEntity<?> createPrize(@RequestHeader("Authorization") String auth, @RequestBody PrizeRequest request) {
        if (getUid(auth) == null) return unauthorized();
        try {
            Prize newPrize = prizeService.createPrize(request);
            return ResponseEntity.status(201).body(newPrize);
        } catch (Exception e) {
